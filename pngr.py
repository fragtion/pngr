...
def render(workers, cfgs, cols, rows, grid_rows=None, grid_cols=None):
    ...
    for i, w in enumerate(workers):
        ...
        snap = w.snap(cell_w)[-cell_w:]
        valid = [x for x in snap if x is not None and not isinstance(x, PENDING)]
        ...
        for x in range(cell_w):
            ...
            if isinstance(val, PENDING):
                continue
            color = BG_LOSS if val is None else BG_WORSE if val > bad else BG_HIGH if val > warn else BG_GOOD
...
def main():
    ...
    try:
        ...
        while True:
            ...
            frame = render(workers, cfgs, cols, rows, grid_rows=args.rows, grid_cols=args.cols)

...
import math
import os
import platform#!/usr/bin/env python3
import argparse
import math
import os
import platform
...
def sample_or_scroll(samples, width):
    count = len(samples)
    if count == 0:
        return [None] * width
...
class Worker(threading.Thread):
    ...
    def _resolve(self, slot, timeout, sequence):
        # Use the shared ICMP manager where available for reliable demultiplexing.
        try:
...
def render(workers, cfgs, cols, rows, grid_rows=None, grid_cols=None, braille=False):
    if braille:
        return render_braille(workers, cfgs, cols, rows, grid_rows=grid_rows, grid_cols=grid_cols)
    return render_highres(workers, cfgs, cols, rows, grid_rows=grid_rows, grid_cols=grid_cols)


def render_highres(workers, cfgs, cols, rows, grid_rows=None, grid_cols=None):
    ...
    for i, w in enumerate(workers):
        ...
        snap = sample_or_scroll(w.snap(min(2000, cell_w)), cell_w)
        valid = [x for x in snap if x is not None]
        ...
        pixel_h = cell_h * 2
        pixels = [[False] * cell_w for _ in range(pixel_h)]
        colors = [BG_BLACK] * cell_w

        for x in range(cell_w):
            val = snap[x]
            colors[x] = value_color(val, warn, bad)
            if val is None:
        ...
        for y in range(cell_h):
            for x in range(cell_w):
                top = pixels[y * 2][x] if y * 2 < pixel_h else False
                bottom = pixels[y * 2 + 1][x] if y * 2 + 1 < pixel_h else False
                if top and bottom:
                    glyph = "█"
                elif top:
                    glyph = "▀"
                ...
                if glyph != " ":
                    canvas[y][x] = BG_BLACK + colors[x] + glyph + RESET
                else:
        ...
        resolved_count = len(snap) - sum(1 for x in snap if isinstance(x, PENDING))
...
def render_braille(workers, cfgs, cols, rows, grid_rows=None, grid_cols=None):
    grid_r, grid_c = layout(len(workers), rows=grid_rows, cols=grid_cols)
    cell_w = max(1, cols // grid_c)
    ...
    for i, w in enumerate(workers):
        ...
        virtual_w = cell_w * 2
        snap = sample_or_scroll(w.snap(min(2000, virtual_w)), virtual_w)
        valid = [x for x in snap if x is not None]
        ...
        colors = [BG_BLACK] * virtual_w

        for vx in range(virtual_w):
            val = snap[vx]
            colors[vx] = value_color(val, warn, bad)
            if val is None:
                for py in range(pixel_h):
                    pixels[py][vx] = True
            else:
        ...
        for y in range(cell_h):
            for x in range(cell_w):
                ...
                if bits:
                    left_color = colors[x * 2] if x * 2 < virtual_w else BG_BLACK
                    right_color = colors[x * 2 + 1] if x * 2 + 1 < virtual_w else left_color
                    severity = {FG_GOOD: 0, FG_HIGH: 1, FG_WORSE: 2, FG_LOSS: 3}
                    fg = left_color if severity.get(left_color, 0) >= severity.get(right_color, 0) else right_color
                    canvas[y][x] = BG_BLACK + fg + braille_from_bits(bits) + RESET
                else:
                    canvas[y][x] = BG_BLACK + " " + RESET
...
def main():
    ap = argparse.ArgumentParser(
        prog="pngr",
        description="pngr v1.0 — terminal ping grapher. Monitor latency and packet loss for one or more hosts in a live grid.",
    ...
    ap.add_argument("--version", action="version", version="pngr v1.0")
    args = ap.parse_args()
